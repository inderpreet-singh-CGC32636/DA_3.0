...
LEFT JOIN analytics_reporting.application_CGCL app
       ON ld.sz_application_no = app.sz_application_no
LEFT JOIN analytics_reporting.applicant_basic_dtl_CGCL apt
       ON ld.sz_application_no = apt.sz_application_no
      AND apt.sz_appl_type_code = 'BORROWER'
LEFT JOIN analytics_reporting.applicant_address_contact_dtl_CGCL addr
...
WHERE ld.loan_status = 'APPROVED'
  ...
  AND (
      UPPER(ld.sz_product_desc) LIKE '%MSME%'
      OR UPPER(ld.sz_portfolio_desc) LIKE '%MSME%'
  )
