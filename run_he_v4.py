# %% Imports & Config
"""
HE DA Pool Selection — 5-Stage Pipeline  (v4)
==============================================
v3 change: MAX_DPD_EVER derived inline from full DPD history (he_dpd_history.sql)
v4 change: #%% cell markers added for interactive VS Code / Jupyter execution

Stage 1  SQL   : simple_he_eligible.sql + he_assets.sql + he_dpd_history.sql
                 + he_bounce.sql + restructure.sql + abhfl_serviceable.sql
Stage 2  Python: age, MOB, CIBIL, LTV, residential_pct, DPD flags (6M/18M/ever),
                 bounce counts, profile_type, seasoning_days, abhfl_serviceable
Stage 3a Python: Bajaj Filters  (live, funder, MOB, DPD, bounce, property, occ, amount, age, tenure, LTV)
Stage 3b Python: ABFL Filters   (live, funder-A, seasoning, sanction, tenure, age, origLTV,
                                  propType, overdue, NPA/DPD, peakDPD, DPD18M, bounce, restructure, pincode)
...
SQL_PATH        = ROOT / "sql" / "simple_he_eligible.sql"
SQL_ASSETS      = ROOT / "sql" / "he_assets.sql"
SQL_DPD         = ROOT / "sql" / "he_dpd_history.sql"   # full history — MAX_DPD_EVER derived here
SQL_BOUNCE      = ROOT / "sql" / "he_bounce.sql"
SQL_RESTRUCTURE = ROOT / "sql" / "restructure.sql"
SQL_SERVICEABLE = ROOT / "sql" / "abhfl_serviceable.sql"
OUT_DIR         = ROOT / "output" / "he_v4"
...
# %% Calc - Base Fields  (age, MOB, CIBIL, profile, seasoning)
...
# %% Calc - Property  (value, residential_pct, plot/shed/comm counts, top asset type)
...
# %% Bajaj Filter Functions  (HE: no UNDER CONS check unlike MSME)
...
# %% Filter - Property  (HE: no plot/shed/vacant, residential >= 80%; no UNDER CONS check)
def filterProperty(df):
    # HE: no UNDER CONS check (differs from MSME)
...
def filterNotFundedABFL(df):
    """ABFL: only blocks funder_status = 'A' (Bajaj blocks any non-null funder)."""
    return df.loc[
...
# %% ABFL Filter - LTV Origination  (<= 70% flat; matches SQL md.ltv = ltv_wo_insurance)
def filterLTVOriginationABFL(df):
    """ABFL HE: origination LTV (ltv_wo_insurance) <= 70%.
    SQL uses md.ltv = app.ltv_wo_insurance — NOT current/calculated LTV.
    NULL ltv_wo_insurance: num() defaults to 999 > 70 → rejected (matches SQL NULL<=70=FALSE).
    """
    return df.loc[
        (num(df, "TOTAL_PROPERTY_VALUE", 0) > 0)
        & (num(df, "LTV_ORIGINATION", 999) <= 70)
    ]

# %% ABFL Filter - Property Type  (res / comm / mix only; no industrial / plot / vacant)
def filterPropertyTypeABFL(df):
    """ABFL: residential / commercial / mix only; no industrial / plot / vacant."""
...
ABFL_FILTER_DICT = {
    ...
    "LTV origination > 70%":                filterLTVOriginationABFL,
...
def _abfl_cibil(df, threshold):
    is_ind = df["SZ_APPL_CATEGORY_CODE"].fillna("").str.upper().str.contains("I")
    return df.loc[
        (is_ind & (num(df, "CIBIL_SCORE", -10) >= threshold))
        | ~is_ind
    ]
...
def main():
    ...
    bajaj_reject_dict = {}
    bajaj_rejected_df = pd.DataFrame(columns=df.columns)
    for reason, func in BAJAJ_FILTER_DICT.items():
        filtered = func(df)
        bajaj_reject_dict, bajaj_rejected_df = checkRejected(
            df, filtered, reason, bajaj_reject_dict, bajaj_rejected_df
        )
    bajaj_accepted = df.loc[~df["LAN"].isin(bajaj_rejected_df["LAN"])].copy()
    print(f"  bajaj filter_pass = {len(bajaj_accepted)}  |  rejected = {len(bajaj_rejected_df)}")
    ...
    abfl_accepted = df.loc[~df["LAN"].isin(abfl_rejected_df["LAN"])].copy()
    print(f"  abfl  filter_pass = {len(abfl_accepted)}  |  rejected = {len(abfl_rejected_df)}")

    # Stage 4: eligibility
    print("Stage 4: applying eligibility criteria...")
    bajaj_eligible = {}
    for name, func in BAJAJ_ELIGIBILITY_DICT.items():
        bajaj_eligible[name] = func(bajaj_accepted)
        print(f"  {name} = {len(bajaj_eligible[name])}")

    abfl_eligible = {}
    for name, func in ABFL_ELIGIBILITY_DICT.items():
        abfl_eligible[name] = func(abfl_accepted)
        print(f"  {name} = {len(abfl_eligible[name])}")

    # Stage 5: output
    print("Stage 5: writing output...")
    df = addRejectionReason(df, bajaj_reject_dict, prefix="BAJAJ")
    df = addRejectionReason(df, abfl_reject_dict,  prefix="ABFL")

    df["BAJAJ_HARD_FILTER_PASS"] = df["LAN"].isin(bajaj_accepted["LAN"]).map({True: "Eligible", False: "Ineligible"})
    df["ABFL_HARD_FILTER_PASS"]  = df["LAN"].isin(abfl_accepted["LAN"]).map({True: "Eligible", False: "Ineligible"})

    for name, elig_df in bajaj_eligible.items():
        df[f"ELIGIBLE_{name}"] = df["LAN"].isin(elig_df["LAN"]).map({True: "Eligible", False: "Ineligible"})
    for name, elig_df in abfl_eligible.items():
        df[f"ELIGIBLE_{name}"] = df["LAN"].isin(elig_df["LAN"]).map({True: "Eligible", False: "Ineligible"})

    out_file = OUT_DIR / f"he_v4_{ts}.csv"
